...
using MexicoTaxReport.Services;

namespace MexicoTaxReport.Data
{
	public partial class DataControllerBase
    {
        ...
        public static ControllerConfiguration CreateConfigurationInstance(Type t, string controller)
        {
            ...
            if (config == null)
            {
                ...
                if (res == DefaultDataControllerStream)
                	res = null;
                if (res == null)
                	res = t.Assembly.GetManifestResourceStream(String.Format("MexicoTaxReport.Controllers.{0}.xml", controller));
                if (res == null)
                	res = t.Assembly.GetManifestResourceStream(String.Format("MexicoTaxReport.{0}.xml", controller));
                if (res == null)
                {
        ...
        public static string CreateReportInstance(Type t, string name, string controller, string view, bool validate)
        {
            ...
            if (t == null)
            	t = typeof(MexicoTaxReport.Data.Controller);
            Stream res = t.Assembly.GetManifestResourceStream(String.Format("MexicoTaxReport.Reports.{0}", name));
            if (res == null)
            	res = t.Assembly.GetManifestResourceStream(String.Format("MexicoTaxReport.{0}", name));
            if (res == null)
            {
            ...
            if (isGeneric)
            {
                ...
                ControllerConfiguration config = MexicoTaxReport.Data.Controller.CreateConfigurationInstance(t, controller);
