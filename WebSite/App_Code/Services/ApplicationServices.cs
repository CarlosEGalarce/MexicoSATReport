...
using MexicoTaxReport.Handlers;
using MexicoTaxReport.Web;

namespace MexicoTaxReport.Services
{
    ...
    public class ApplicationServicesBase
    {
        ...
        {
            MexicoTaxReport.Services.ApplicationServices.Initialize();
        }
        ...
        public static void RegisterStandardMembershipAccounts()
        {
            ...
            if (Membership.GetUser("admin") == null)
            {
                ...
                admin = Membership.CreateUser("admin", "admin123%", "admin@MexicoTaxReport.com", "ASP.NET", "Code OnTime", true, out status);
                user = Membership.CreateUser("user", "user123%", "user@MexicoTaxReport.com", "ASP.NET", "Code OnTime", true, out status);
        ...
        public static void RegisterCssLinks(Page p)
        {
            if (ApplicationServices.IsTouchClient)
            {
                ...
                foreach (string fileName in TouchUIStylesheets())
                	if (!(fileName.StartsWith("bootstrap")))
                    {
                        ...
                        if (fileName.Contains("app-themes."))
